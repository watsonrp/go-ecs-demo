...
func (this *MainController) Login() {
	...
	if this.Ctx.Input.Method() == "POST" {
		...
		o := orm.NewOrm()
		o.Using("default")
		user := models.LocalUser{Email: email}
...
func (this *MainController) Register() {
	...
	if this.Ctx.Input.Method() == "POST" {
		...
		user := models.LocalUser{First: first, Last: last, Email: email}
...
func sendVerification(email, u string) bool {
	...
	msg.SetAddressHeader("From", "techlabsnz@gmail.com", "techlabs Site")
	msg.SetHeader("To", email)
	msg.SetHeader("Subject", "Account Verification for techlabs Corporation")
	msg.SetBody("text/html", "To verify your account, please click on the link: <a href=\""+link+
		"\">"+link+"</a><br><br>Best Regards,<br>Techlabs Site")
	m := gomail.NewMailer(host, "techlabsnz@gmail.com", "sYDVp1R8X2kj", port)
...
func (this *MainController) Verify() {
	...
	o := orm.NewOrm()
	o.Using("default")
	user := models.LocalUser{RegKey: u}
	err := o.Read(&user, "RegKey")
	if err == nil {
...
func (this *MainController) Profile() {
	...
	o := orm.NewOrm()
	o.Using("default")
	user := models.LocalUser{Email: m["username"].(string)}
	err := o.Read(&user, "Email")
	if err == nil {
		// scan in the password hash/salt
		if x.Hash, err = hex.DecodeString(user.Password[:64]); err != nil {
	...
	defer func(this *MainController, user *models.LocalUser) {
